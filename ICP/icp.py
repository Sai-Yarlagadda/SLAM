...
def find_projective_correspondence(source_points,
    ...
    #conditions u,v,d must satisfy---->
    # w>target_us>=0
    # h>target_vs>=0
    # inf>depth>=0
    mask = ((target_us<w) & (target_us>=0) & (target_vs<h) & (target_vs>=0) & (target_ds >= 0)).astype(bool)
    ...
    #Condition to be satisfied---------->|p-q|<dist_threshold
    q = target_vertex_map[target_vs, target_us]
    diff_sq = np.sum((T_source_points - q)**2, axis=1)
    diff = np.sqrt(diff_sq)

    mask = (diff<dist_diff).astype(bool)
    # End of TODO
...
def build_linear_system(source_points, target_points, target_normals, T):
    ...
    #n_q[i] = [n_qx, n_qy, n_qz]
    for i in range(M):
        #A matrix
        Ai0 = n_q[i,2]*p_prime[i,1] - n_q[i,1]*p_prime[i,2]
        Ai1 = n_q[i,0]*p_prime[i,2] - n_q[i,2]*p_prime[i,0]
        Ai2 = n_q[i,1]*p_prime[i,0] - n_q[i,0]*p_prime[i,1]
        Ai3 = n_q[i,0]
        Ai4 = n_q[i,1]
        Ai5 = n_q[i,2]
        Ai = [Ai0, Ai1, Ai2, Ai3, Ai4, Ai5]
        A[i] = Ai

        #B matrix
        bi0 = n_q[i,0]*q[i,0] + n_q[i,1]*q[i,1] + n_q[i,2]*q[i,2] - n_q[i,0]*p_prime[i,0]- n_q[i,1]*p_prime[i,1]- n_q[i,2]*p_prime[i,2]
        b[i] = bi0
