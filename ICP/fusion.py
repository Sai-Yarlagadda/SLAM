...
class Map:
    ...
    def merge(self, indices, points, normals, colors, R, t):
        ...
        '''
        ck = 1
        alpha = self.weights[indices]
        self.points[indices] = (alpha*self.points[indices] + ((R @ points.T +t).T))/(alpha+ck)
        
        normal_without_norm = (alpha*self.normals[indices] + ((R @ normals.T).T))/(alpha+ck)
        norm = np.linalg.norm(normal_without_norm, axis=1)
        #print(norm)
        self.normals[indices] = self.normals[indices]/norm[0]

        self.colors[indices] = (alpha*self.colors[indices] + colors)/(alpha + ck)
    ...
    def add(self, points, normals, colors, R, t):
        ...
        self.points = np.concatenate((self.points, (M).T))
        self.normals = np.concatenate((self.normals, (N).T))
    ...
    def filter_pass1(self, us, vs, ds, h, w):
        ...
        '''
        mask = ((us<w) & (us>=0) & (vs<h) & (vs>=0) & (ds>=0)).astype(bool)
    ...
    def filter_pass2(self, points, normals, input_points, input_normals,
        ...
        '''
        dist_sq = np.sum((points-input_points)**2, axis=1)
        dist = np.sqrt(dist_sq)
        
        #dot product/magnitude = cos theta
        dot_product = np.sum((normals*input_normals), axis=1)
        mag_normal = np.linalg.norm(normals, axis=1)
        mag_input_normal = np.linalg.norm(input_normals, axis=1)
        cos_theta = dot_product/((mag_normal)*(mag_input_normal))
        theta = np.abs(np.arccos(cos_theta))
        mask = ((dist<dist_diff) & (theta<angle_diff)).astype(bool)
...
if __name__ == '__main__':
    ...
    intrinsic_struct = o3d.io.read_pinhole_camera_intrinsic('C:\\Subjects\\Subjects\\SLAM\\Assignments\\Assignment4\\16833_HW4_ICP_Fusion\\code\\intrinsics.json')
