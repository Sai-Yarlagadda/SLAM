...
def compute_meas_obs_jacobian(x, i, j, n_poses):
    ...
    #print(j)
    #print(dist)
    jacobian[0,0] =(dy)/dist**2
    jacobian[1,0] = -(dx)/dist
    jacobian[0,1] = -1*(dx)/dist**2
    jacobian[1,1] = -1*(dy)/dist
    jacobian[0,2] = -1*(dy)/dist**2
    jacobian[1,2] = (dx)/dist
    jacobian[0,3] = (dx)/dist**2
    jacobian[1,3] = (dy)/dist
    return jacobian
...
def create_linear_system(x, odoms, observations, sigma_odom, sigma_observation,
    ...
    for i in range(n_odom):
        ...
        B_odom = sqrt_inv_odom@(odoms[i] - odometry_estimation(x,i))
    ...
    for i in range(n_obs):
        ...
        landmark_idx = int(observations[i,1])
        #print(landmark_idx)
        h_land = compute_meas_obs_jacobian(x, poses_idx, landmark_idx, n_poses)
        ...
        z2 = observations[i,3]
        #z = np.array([z1,z2])
        bearing  = bearing_range_estimation(x,poses_idx,landmark_idx, n_poses)
        theta = warp2pi(z1 - bearing[0])
        r = (z2 - bearing[1])
        k = np.array([theta, r])
        B_land = (sqrt_inv_obs) @ (k)
