...
def init_particles_freespace(num_particles, occupancy_map):
    # Find the coordinates of free space in the occupancy map
    free_space_coords = np.argwhere(occupancy_map == 0)
    num_free_space_coords = free_space_coords.shape[0]

    # Generate random indices to sample from free space coordinates
    random_indices = np.random.choice(num_free_space_coords, num_particles, replace=True)

    # Sample random coordinates from free space and create particle positions
    sampled_coords = free_space_coords[random_indices]

    # Create arrays for x, y, and theta values
    ...
    # Combine x, y, theta, and weight values into a single array
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', default='results3')
